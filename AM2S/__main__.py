...
def main():
    ...
    for folder in scannedFolders:
        ...
        for romset in romsets:
            ...
            for rom in romset.games:
                try:
                    ...
                    with alive_bar(
                        ...
                        progress.title(f"{taskPrefix} {rom.name} - Formatting description")
            ...
            Dt.success(f"{len(romset.games) - errorCount} ROM(s) for {romset.console.genericName} has been scrapped !")
            if isLenient and errorCount > 0:
                Dt.warning(f"{errorCount} error(s) occurred. The following ROMs have not been scrapped:")
                for file in errors:
                    print(f"\t- {file}")

    endTime = time.time()
    print("")
    Dt.success(f"All done ! It took a total of {round(endTime - startTime, 2)} seconds. Enjoy <3")
