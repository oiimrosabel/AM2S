...
class GameMediaGetter:
	...
	def downloadBoxArt(self, path: Path):
		...
		screenDims: list[int] = self.__config.get(
			"images/dimensions/screenDims", [640, 480]
		)
		barsSizes: list[int] = self.__config.get(
			"images/dimensions/barsSizes", [43, 42]
		)
		padding: (int, int) = self.__config.get(
			"images/dimensions/padding", [12, 12]
		)
		withCartridge: bool = self.__config.get(
			"images/filters/withCartridge", False
		)
	...
	def __applyFilters(self, background: Image) -> Image:
		brightness: float = self.__config.get("images/filters/brightness", 1.0)
	...
	def __retrieveCartridgeImage(self, path: Path) -> Image:
		try:
			...
			cartridgeImage = self.__resizeToWidth(
				cartridgeImage, self.__medias.rom.console.preferredWidth
			)
		except:
	...
	@staticmethod
	def __addCartridgeToImage(
		background: Image, cartridge: Image, padding: list[int]
	) -> Image:
